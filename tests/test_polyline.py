...
class TestSimplePolyline:
    ...
    @pytest.mark.parametrize("points", [
        MeasurePoint(
            randint(-100, 100), 31, randint(-1000, 10000)
    ...
    @pytest.mark.parametrize("points", [
        MeasurePoint(
            randint(-100, 100), -1, randint(-1000, 10000)
        ) for i in range(0, random_values_to_test)
    ])
    ...
    @pytest.mark.parametrize("points", [
        MeasurePoint(
            randint(-100, 100), 0, randint(-1000, 1000)
        ) for i in range(0, random_values_to_test)
    ])
    ...
    @pytest.mark.parametrize("points", [
        MeasurePoint(
            randint(-100, 100), 30, randint(-1000, 1000)
        ) for i in range(0, random_values_to_test)
    ])
    ...
    @pytest.mark.parametrize("points", [
        MeasurePoint(
            randint(-100, 100), randint(1, 9), randint(-1000, 10000)
        ) for i in range(0, random_values_to_test)
    ])
    ...
    @pytest.mark.parametrize("points", [
        MeasurePoint(randint(-100, 100), randint(21, 29), randint(-1000, 10000)) for i in
        range(0, random_values_to_test)
    ])
...
class TestComplexPolyline:
    ...
    def complex_3d_line(self):
        string_data = "161063.87,383689.249,20.346 160996.307,383682.013,20.475 160980.713,383680.732,20.51 " \
    ...
    def rail_connection_75t_line(self):
        ...
        track_data = "161756.813,383773.874,18.761 161940.82,383814.894,18.336 161964.502,383820.223,18.289 161984.591,383824.984,18.262 162002.608,383829.097,18.23 162007.296,383830.153,18.226 162051.151,383840.028,18.189 162351.515,383906.741,17.875 162361.406,383908.6,17.885 162378.35,383912.147,17.856 162404.107,383915.76,17.668"
        ...
        passage_3_data = "162440.463,383918.65,17.668 162429.966,383917.815,17.668 162404.107,383915.76,17.668"
    ...
    def test_rail_connection_75t_measures(self, rail_connection_75t_line, points):
        tester = rail_connection_75t_line[0].project(
            MeasurePoint(points[0], points[1], points[2])
        )
